...
export class TenantService {
  ...
  async validateNas(
    ...
    this.logger.log(
      `Validating NAS: nasId=${nasId}, bssid=${bssid}, publicIp=${publicIp}`,
    );
  ...
  ): Promise<Prisma.TenantGetPayload<{}>> {
  ...
  async findStaff(slug: string): Promise<any[]> {
    ...
    if (!tenant) {
      console.warn(
        `[TenantService.findStaff] Tenant not found for slug: ${slug}`,
      );
      return [];
    }
    ...
    if (staff.length === 0) {
      this.logger.warn(
        `[TenantService.findStaff] No members found for tenant "${slug}". Falling back to first user for dev/demo purposes.`,
      );
    ...
    console.log(
      `[TenantService.findStaff] Found ${staff.length} staff members for tenant "${slug}". Primary:`,
      staff[0] ? { id: staff[0].id, name: staff[0].name } : 'None',
    );
    return staff;
  }
  ...
  async createFeedback(
    ...
    if (!userExists) {
      this.logger.log(
        `[createFeedback] User ${userId} not found, creating anonymous user`,
      );
      try {
        ...
        this.logger.error(
          `Failed to create anonymous user for feedback: ${e.message}`,
        );
