...
export class TenantController {
  ...
  async submitFeedback(
    ...
    return this.tenantService.createFeedback(
      userId,
      tenant.id,
      body.score,
      body.comment,
  ...
  private async resolveUserId(auth: string): Promise<string> {
    ...
    try {
      ...
      const resolvedUserId =
        await this.tenantService.resolveUserFromToken(token);
      if (resolvedUserId) {
  ...
  private async handleValidateNas(
    request: any,
    data: { nasId?: string; bssid?: string; publicIp?: string },
  ) {
    // ... (existing code)
    const forwarded = request.headers['x-forwarded-for'];
    const remoteIp = forwarded
    ...
    console.log(
      `[TenantController] Validating NAS: nasId=${data.nasId}, bssid=${data.bssid}, IP=${publicIp}`,
    );
